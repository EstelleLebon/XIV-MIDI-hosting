...
class File {
	...
	async init() {
		try {
			...
			if (!buffer) {
				...
				this.error.addError(`[FILECLASS] [INIT] Buffer is null`);
			}
			...
			if (!md5) {
				...
				this.error.addError(`[FILECLASS] [INIT] MD5 is null`);
			}
			...
			this.logger.error(`[INIT] Error initializing file: ${error}`);
			this.error.addError(`[FILECLASS] [INIT] Error initializing file: ${error}`);
			this.initialized = false;
		}
	...
	async init_from_interaction() {
		try {
			...
			if (!this.interaction) {
				...
				this.error.addError(`[FILECLASS] [init_from_interaction] Error: Interaction is null`);
			}
			if (!this.interaction.options) {
				this.logger.error(`[init_from_interaction] Options are null`);
				this.error.addError(`[FILECLASS] [init_from_interaction] Error: Options are null`);
			}
			if (!this.interaction.user) {
				this.logger.error(`[init_from_interaction] User is null`);
				this.error.addError(`[FILECLASS] [init_from_interaction] Error: User is null`);
			}
			if (!this.interaction.user.id) {
				this.logger.error(`[init_from_interaction] User id is null`);
				this.error.addError(`[FILECLASS] [init_from_interaction] Error: User id is null`);
			}
			...
			if (!this.interaction.user.tag) {
				...
				this.error.addError(`[FILECLASS] [init_from_interaction] Error: User tag is null`);
			}
			...
			if (!editor_discord_id) {
				...
				this.error.addError(`[FILECLASS] [init_from_interaction] Error: Editor discord id is null`);
			}
			...
			if (!artist) {
				...
				this.error.addError(`[FILECLASS] [init_from_interaction] Error: Artist is null`);
			}

			const title = await this.init_title();
			if (!title) {
				this.logger.error(`[init_from_interaction] Title is null`);
				this.error.addError(`[FILECLASS] [init_from_interaction] Error: Title is null`);
			}

			const performer = await this.init_performer();
			if (!performer) {
				this.logger.error(`[init_from_interaction] Performer is null`);
				this.error.addError(`[FILECLASS] [init_from_interaction] Error: Performer is null`);
			}

			const sources = await this.init_sources();
			if (!sources) {
				this.logger.error(`[init_from_interaction] Sources are null`);
				this.error.addError(`[FILECLASS] [init_from_interaction] Error: Sources are null`);
			}

			const comments = await this.init_comments();
			if (!comments) {
				this.logger.error(`[init_from_interaction] Comments are null`);
				this.error.addError(`[FILECLASS] [init_from_interaction] Error: Comments are null`);
			}

			const tags = await this.init_tags();
			if (tags === null || tags === undefined) {
				this.logger.debug(`[init_from_interaction] Tags are null`);
				this.error.addError(`[FILECLASS] [init_from_interaction] Error: Tags are null`);
			}
		} catch (error) {
			this.logger.error(`[init_from_interaction] Error initializing file from interaction: ${error}`);
			this.error.addError(`[FILECLASS] [init_from_interaction] Error initializing file from interaction: ${error}`);
		}
	}
	...
	async init_buffer() {
		try {
			if (!this.interaction) {
				...
				this.error.addError(`[FILECLASS] [init_buffer] Error: Interaction is null`);
				return null;
			}
			if (!this.interaction.options) {
				this.logger.error(`[init_buffer] Options are null`);
				this.error.addError(`[FILECLASS] [init_buffer] Error: Options are null`);
				return null;
			}
			this.logger.debug(`[init_buffer] Getting buffer from interaction`);
			var midi = this.interaction.options.getAttachment('midi');
			if (!midi) {
				this.logger.error(`[init_buffer] Midi is null`);
				this.error.addError(`[FILECLASS] [init_buffer] Error: Midi is null`);
				return null;
			}
			if (!midi.url) {
				this.logger.error(`[init_buffer] Midi url is null`);
				this.error.addError(`[FILECLASS] [init_buffer] Error: Midi url is null`);
				return null;
			}
			...
			if (!this.buffer) {
				...
				this.error.addError(`[FILECLASS] [init_buffer] Error: Buffer is null`);
				return null;
			}
			this.logger.debug(`[init_buffer] Buffer size: ${this.buffer.length}`);
			return this.buffer;
		} catch (error) {
			this.logger.error(`[init_buffer] Error getting buffer: ${error}`);
			this.error.addError(`[FILECLASS] [init_buffer] Error: Error getting buffer: ${error}`);
			return null;
		}
	}
	...
	async init_md5() {
		if (!this.buffer) {
			...
			this.error.addError(`[FILECLASS] [init_md5] Error: Buffer is null`);
			return null;
		}
		try {
			...
			this.logger.error(`[init_md5] Error getting md5: ${error}`);
			this.error.addError(`[FILECLASS] [init_md5] Error: Error getting md5: ${error}`);
			return null;
		}
	}
	...
	async init_artist() {
		if (!this.interaction) {
			...
			this.error.addError(`[FILECLASS] [init_artist] Error: Interaction is null`);
			return null;
		}
		if (!this.interaction.options) {
			this.logger.error(`[init_artist] Options are null`);
			this.error.addError(`[FILECLASS] [init_artist] Error: Options are null`);
			return null;
		}
		try {
			...
			if (!artist) {
				...
				this.error.addError(`[FILECLASS] [init_artist] Error: Artist is null`);
				return null;
			}
			...
			this.logger.error(`[init_artist] Error getting artist: ${error}`);
			this.error.addError(`[FILECLASS] [init_artist] Error: Error getting artist: ${error}`);
			return null;
		}
	}
	...
	async init_title() {
		if (!this.interaction) {
			...
			this.error.addError(`[FILECLASS] [init_title] Error: Interaction is null`);
			return null;
		}
		if (!this.interaction.options) {
			this.logger.error(`[init_title] Options are null`);
			this.error.addError(`[FILECLASS] [init_title] Error: Options are null`);
			return null;
		}
		...
		try {
			...
			if (!title) {
				...
				this.error.addError(`[FILECLASS] [init_title] Error: Title is null`);
				return null;
			}
			...
			this.logger.error(`[init_title] Error getting title: ${error}`);
			this.error.addError(`[FILECLASS] [init_title] Error: Error getting title: ${error}`);
			return null;
		}
	}
	...
	async init_performer() {
		if (!this.interaction) {
			...
			this.error.addError(`[FILECLASS] [init_performer] Error: Interaction is null`);
			return null;
		}
		if (!this.interaction.options) {
			this.logger.error(`[init_performer] Options are null`);
			this.error.addError(`[FILECLASS] [init_performer] Error: Options are null`);
			return null;
		}
		try {
			...
			if (!performer) {
				...
				this.error.addError(`[FILECLASS] [init_performer] Error: Performer is null`);
				return null;
			}
			...
			this.logger.error(`[init_performer] Error getting performer: ${error}`);
			this.error.addError(`[FILECLASS] [init_performer] Error: Error getting performer: ${error}`);
			return null;
		}
	}
	...
	async init_sources() {
		if (!this.interaction) {
			...
			this.error.addError(`[FILECLASS] [init_sources] Error: Interaction is null`);
			return null;
		}
		if (!this.interaction.options) {
			this.logger.error(`[init_sources] Options are null`);
			this.error.addError(`[FILECLASS] [init_sources] Error: Options are null`);
			return null;
		}
		try {
			...
			if (typeof sources !== 'string') {
				...
				this.error.addError(`[FILECLASS] [init_sources] Error: Sources is not a string: ${typeof sources}`);
				return null;
			}
			...
			this.logger.error(`[init_sources] Error getting sources: ${error}`);
			this.error.addError(`[FILECLASS] [init_sources] Error: Error getting sources: ${error}`);
			return null;
		}
	}
	...
	async init_comments() {
		if (!this.interaction) {
			...
			this.error.addError(`[FILECLASS] [init_comments] Error: Interaction is null`);
			return null;
		}
		if (!this.interaction.options) {
			this.logger.error(`[init_comments] Options are null`);
			this.error.addError(`[FILECLASS] [init_comments] Error: Options are null`);
			return null;
		}
		try {
			...
			this.logger.error(`[init_comments] Error getting comments: ${error}`);
			this.error.addError(`[FILECLASS] [init_comments] Error: Error getting comments: ${error}`);
			return null;
		}
	}
	...
	async init_tags() {
		...
		if (!this.interaction) {
			...
			this.error.addError(`[FILECLASS] [init_tags] Error: Interaction is null`);
			return null;
		}
		if (!this.interaction.options) {
			this.logger.error(`[init_tags] Options are null`);
			this.error.addError(`[FILECLASS] [init_tags] Error: Options are null`);
			return null;
		}
		try {
			...
			this.logger.error(`[init_tags] Error getting tags: ${error}`);
			this.error.addError(`[FILECLASS] [init_tags] Error: Error getting tags: ${error}`);
			return null;
		}
	}
	...
	async init_pushes() {
		try {
			...
			this.logger.error(`[init_pushes] Error getting pushes: ${error}`);
			this.error.addError(`[FILECLASS] [init_pushes] Error: Error getting pushes: ${error}`);
			return null;
		}
	}
	...
	async init_final_pushes() {
		if (this.interaction) {
			try {
				...
				this.logger.error(`[init_final_pushes] Error getting pushes: ${error}`);
				this.error.addError(`[FILECLASS] [init_final_pushes] Error: Error getting pushes: ${error}`);
				return null;
			}			
		} else {
			this.logger.error(`[init_final_pushes] Interaction is null`);
			this.error.addError(`[FILECLASS] [init_final_pushes] Error: Interaction is null`);
			return null;
		}
		return;
	}
	...
	async init_filename() {
		try {
			...
			this.logger.error(`[init_filename] Error getting filename: ${error}`);
			this.error.addError(`[FILECLASS] [init_filename] Error: Error getting filename: ${error}`);
			return null;
		}
	}
	...
	async push(preview) {
		try {
			...
			if (this.error.iserror()) {
				this.logger.error(`[push] Errors detected during push`);
				this.error.addError(`[FILECLASS] [push] Error: Errors detected during push`);
			...
			this.logger.error(`[push] Error pushing file: ${error}`);
			this.error.addError(`[FILECLASS] [push] Error: Error pushing file: ${error}`);
			clearInterval(preview.upload_interval);
			await preview.embed.updateEmbedField('Status: ', 'Failed');
			await preview.updatePreview();
			return null;
	...
	async push_discord(preview) {
		try {
			...
			let channel = await client.default.channels.fetch(channelID);
			if (!channel) {
				this.logger.error(`[push_discord] Channel not found`);
				this.error.addError(`[FILECLASS] [push_discord] Error: Channel not found`);
				return null;
			}
			...
			this.logger.error(`[push_discord] Error pushing file to discord: ${error}`);
			this.error.addError(`[FILECLASS] [push_discord] Error: Error pushing file to discord: ${error}`);
			await preview.embed.updateEmbedField('discord push:', 'Error');
			clearInterval(preview.upload_interval);
			await preview.embed.updateEmbedField('Status: ', 'Failed');
			await preview.updatePreview();
			return null;
	...
	async push_website(preview) {
		try {
			this.logger.info(`[push_website] Pushing file to website`);
			...
			await this.push_website_file(preview);
			...
			this.logger.error(`[push_website] Error pushing file to website: ${error}`);
			this.error.addError(`[FILECLASS] [push_website] Error: Error pushing file to website: ${error}`);
			await preview.embed.updateEmbedField('website push:', 'Error');
			clearInterval(preview.upload_interval);
			await preview.embed.updateEmbedField('Status: ', 'Failed');
			await preview.updatePreview();
			return null;
	...
	async push_website_file(preview) {
		...
		try {
			...
			this.website_file_path = '/files' + finalPath.replace(remove, '');
			...
			this.logger.error(`[push_website_file] Error pushing file to website: ${error}`);
			this.error.addError(`[FILECLASS] [push_website_file] Error: Error pushing file to website: ${error}`);
			await preview.embed.updateEmbedField('website push:', 'Error');
			clearInterval(preview.upload_interval);
			await preview.embed.updateEmbedField('Status: ', 'Failed');
			await preview.updatePreview();
	...
	async push_editor_channel(preview) {
		try {
			...
			this.logger.error(`[push_editor_channel] Error pushing to editor channel: ${error}`);
			await preview.embed.updateEmbedField('Editor Channel Push:', 'Error');
			clearInterval(preview.upload_interval);
			await preview.embed.updateEmbedField('Status: ', 'Failed');
			await preview.updatePreview();
		}
			...
			if (this.existfile) {
				...
				const response = await axios.put(`${discordbdd}/files`, data)
					.catch(async (error) => {
						...
						this.error.addError(`[FILECLASS] [push_database] Error: Error pushing file to database: ${error}`);
						clearInterval(preview.upload_interval);
						await preview.embed.updateEmbedField('Status: ', 'Failed');
						await preview.updatePreview();
						return null;
		...
		try {
			...
			const response = await axios.get(`${discordbdd}/files?${filter}`)
				.catch((error) => {
					...
					this.error.addError(`[FILECLASS] [get_files] Error: Error getting files: ${error}`);
					return null;
				});
	...
	async get_file_and_init(query) {
		try {
			...
			this.logger.error(`[get_file_and_init] Error getting file from database: ${error}`);
			this.error.addError(`[FILECLASS] [get_file_and_init] Error: Error getting file from database: ${error}`);
			return null;
		}
	}
