...
class User {
	...
	async init() {
		try {
			...
			if (this.interaction) {
				...
				this.error.addError(`[USERCLASS] [INIT] Error: this.interaction: ${this.interaction ? this.interaction : '????'}`);
			}
			...
			if (this.discord_id) {
				...
				this.error.addError(`[USERCLASS] [INIT] Error: this.discord_id: ${this.discord_id ? this.discord_id : '????'}`);
			}

			this.logger.info(`[INIT] User ${this.discord_id} initialized`);
			this.initialized = true;
		} catch (error) {
			this.logger.error(`[INIT] Error initializing user: ${error}`);
			this.error.addError(`[USERCLASS] [INIT] Error: ${error}`);
		}
	...
	async handle_editor_name(cachedname) {
		try {
				...
				if (name && name != '' && name != ' ') {
					if (name != cachedname) {
						...
						.catch((error) => {
							...
							this.error.addError(`[USERCLASS] [handle_editor_name] Error: ${error}`);
							return
						});
						return;
					}
			...
			this.logger.error(`[handle_editor_name] Error handling editor name: ${error}`);
			this.error.addError(`[USERCLASS] [handle_editor_name] Error: ${error}`);
		}
		return;
	}
	...
	async init_from_interaction() {
		try {
			...
			if (!this.discord_id) {
				...
				this.error.addError(`[USERCLASS] [init_from_interaction] Error: this.discord_id: ${this.discord_id}`);
				return null;
			}
			if (!this.interaction) {
				this.logger.error(`[init_from_interaction] Interaction is null`);
				this.error.addError(`[USERCLASS] [init_from_interaction] Error: this.interaction: ${this.interaction}`);
				return null;
			}
			...
			this.logger.error(`[init_from_interaction] Error updating user from interaction: ${error}`);
			this.error.addError(`[USERCLASS] [init_from_interaction] Error: ${error}`);
		}
		return;
	}
	...
	async get_user() {
		try {
			...
			if (!dbStatus) {
				this.logger.error('Database is down, aborting create user.');
				return;
			}
			...
			if (!this.discord_id) {
				...
				this.error.addError(`[USERCLASS] [get_user] Error: this.discord_id: ${this.discord_id}`);
				return null;
			}
			...
			const userdata = await axios.get(`${discordbdd}/users/${this.discord_id}`)
				.catch((error) => {
					...
					// this.error.addError(`Error getting user`);
					this.logger.debug(`[get_user] User data: ${JSON.stringify(userdata, null, 2)}`);
					this.error.addError(`[USERCLASS] [get_user] Error: ${error}`);
					return null;
				});
			...
			this.logger.error(`[get_user] Error getting user: ${error}`);
			this.error.addError(`[USERCLASS] [get_user] Error: ${error}`);
		}
		return;
	}
	...
	async create_user() {
		try {
			...
			if (!this.discord_id) {
				...
				// this.error.addError(`Error: this.discord_id: ${this.discord_id}`);
				return null;
			}
			if (!this.initialized) {
				this.logger.error(`[create_user] User ${this.discord_id} not initialized`);
				// this.error.addError(`User not initialized`);
				return null;
			}
			const data = {
			...
			await axios.post(`${discordbdd}/users`, data)
				...
				.catch((error) => {
					...
					this.error.addError(`[USERCLASS] [create_user] Error: ${error}`);
					return;
				});
		
		} catch (error) {
			this.logger.error(`[create_user] Unexpected error: ${error}`);
			this.error.addError(`[USERCLASS] [create_user] Error: ${error}`);
		}
	}
	...
	async update_user({ discord_name = null, editor_name = null, admin = null, editor_role = null, editor_channel_id = null } = {}) {
		try {
			...
			if (!existingUser) {
				this.logger.error(`[update_user] User ${this.discord_id} not found`);
				// this.error.addError(`User not found`);
				return;
			}
			...
			await axios.put(`${discordbdd}/users`, data)
				...
				.catch((error) => {
					...
					this.error.addError(`[USERCLASS] [update_user] Error: ${error}`);
					return;
				});
		} catch (error) {
			this.logger.error(`[update_user] Error updating user: ${error}`);
		...
		try {
			...
			await axios.delete(`${discordbdd}/users/${this.discord_id}`)
				...
				.catch((error) => {
					...
					this.error.addError(`[USERCLASS] [delete_user] Error: ${error}`);
					return;
				});
		} catch (error) {
			this.logger.error(`[delete_user] Error deleting user: ${error}`);
	...
	async update_editor_channel_id() {
		try {
			...
			if (!categoryId) {
				...
				this.error.addError('Category ID not found');
				return null;
			}
			...
			if (!interaction) {
				...
				this.error.addError('Interaction is null');
				return null;
			}
			...
			try {
				...
				this.error.addError(`Error fetching category: ${error}`);
				return null;
			}
			...
			this.logger.error(`[update_editor_channel_id] Error updating editor channel ID: ${error}`);
			this.error.addError(`[USERCLASS] [update_editor_channel_id] Error: ${error}`);
		}
		return;
	}
